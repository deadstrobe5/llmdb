...
def init_db(tables_to_include):
    """Initialize the vector database with selected tables"""
    load_dotenv()
    ...
    tables_to_process = [table for table in tables_to_include if table in all_tables]
    
    if not tables_to_process:
        print("None of the specified tables were found in the database")
        return False
    ...
    for table_name in tables_to_process:
        ...
        processed_count += 1
        if processed_count % 5 == 0 or processed_count == len(tables_to_process):
            print(f"Progress: {processed_count}/{len(tables_to_process)} tables", end="\r")
...
if __name__ == "__main__":
    ...
    print("Initializing vector database with selected tables...")
    success = init_db(selected_tables)
